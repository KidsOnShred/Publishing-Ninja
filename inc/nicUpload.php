...
define ('SERVER_DIRECTORY','/home/angeltes/public_html/publisher/');
define ('SERVER_URL','http://www.angel-test.net/publisher/');

define('NICUPLOAD_PATH', SERVER_DIRECTORY . 'content/img/'); // Set the path (relative or absolute) to
                                      // the directory to save image files
                                      
define('NICUPLOAD_URI', SERVER_URL . 'content/img');   // Set the URL (relative or absolute) to
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    $image = $file['tmp_name'];
	$image = $_FILES["nicImage"]["tmp_name"];
	...
	$path =  SERVER_DIRECTORY . "content/img/" . $filename;
	$actualpath = SERVER_URL . "content/img/" .$filename;
	...
	resizeImage($image, $path, $width, $height);			
...
function resizeImage(&$tempImage, $filepath, &$width, &$height)
{
	...
	imagejpeg($newImage, $filepath, 44);
	$tempImage = $newImage;
	$width = $newWidth;
	$height = $newHeight;
	return true;
