...
var bkEvent = {
	...
	fireEvent : function() {
		...
		/*if (evType == 'selected' && typeof(arguments[1]) != 'undefined') {
			var string = arguments[1]['elm'].innerHTML;
			string = string.replaceAll("<div>", "<p>");
			string = string.replaceAll("</div>", "</p>");
			arguments[1]['elm'].innerHTML = string;
			console.log(string);
		}*/
	}	
};
...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '/publisher/img/nicEditorIcons.gif',
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var editorElm = new bkElement('DIV').setStyle({width : (newX-60)+'px', margin: '20px', minHeight : newY+'px'}).addClass('main').appendTo(this.editorContain);
			...
			this.elm = editorElm;
			if (this.elm.innerHTML.length == 0)
	...
	mouseClick : function() {
		if(!this.isDisabled) {
			if(this.pane && this.pane.pane) {
				...
				this.pane = new nicEditorPane(this.contain,this.ne,{width : (this.width || '400px'), backgroundColor : '#fff'},this);
				this.addPane();
...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			...
			'href' : {type : 'text', txt : 'URL', value : 'http://', style : {width: '300px'}},
			'title' : {type : 'text', txt : 'Title'},
			'target' : {type : 'select', txt : 'Open In', options : {'' : 'Current Window', '_blank' : 'New Window'},style : {width : '300px'}}
		},this.ln);
	},
...
var nicImageButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			...
			'src' : {type : 'text', txt : 'URL', 'value' : 'http://', style : {width: '300px'}},
			'alt' : {type : 'text', txt : 'Alt Text', style : {width: '300px'}},
			'width' : {type : 'select', txt : 'Width', style : {width: '300px'}, options : {'100%' : '100%', '70%' : '70%', '60%' : '60%', '50%' : '50%', '40%' : '40%', '30%' : '30%'}},
			'align' : {type : 'select', txt : 'Align', style : {width: '300px'}, options : {none : 'Default','left' : 'Left', 'right' : 'Right'}}
		},this.im);
	},
	...
	submit : function(e) {
		...
		if (this.im) {
			$(this.im).prepend('<div style="width:'+this.inputs['width'].value+'">');
			$(this.im).append('</div>');
		}
	}
});
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	nicURI : '/publisher/inc/nicUpload.php',
	...
	addPane : function() {
		...
		this.myFrame = new bkElement('iframe').setAttributes({ width : '100%', height : '300px', frameBorder : 0, scrolling : 'no' }).setStyle({border : 0}).appendTo(this.pane.pane);
	...
	addForm : function() {
		...
		myDoc.write('<div><select name="type" class="uploadType"><option value="0">Image</option><option value="1">Figure</option></select></div>');
		myDoc.write('<div><select name="width" class="uploadWidth"><option value="">Default</option><option value="w-66">66%</option><option value="w-60">60%</option><option value="w-50">50%</option><option value="w-40">40%</option><option value="w-33">33%</option></select></div>');
		myDoc.write('<div><select name="align" class="uploadAlign"><option value="0">Left</option><option value="1">Right</option></select></div>');
	...
	startUpload : function() {
		...
		this.myStatus.setContent('<strong>Uploading...</strong>');
		this.myForm.submit();
	...
	update : function(o) {
		if(o == false) {
			...
			if(!this.im) {
				...
				this.im = this.findElm('IMG','src',tmp);
				$(this.im).after('<p></p>');
				var classWidth = $(this.myForm).find('.uploadWidth option').filter(":selected").val();
				var imgType = $(this.myForm).find('.uploadType option').filter(":selected").val();
				var imgAlign = $(this.myForm).find('.uploadAlign option').filter(":selected").val();
				if (imgAlign == '1')
					classWidth += ' float-r';
				if (imgType == '1')
					$(this.im).wrap('<div class="img-figure '+classWidth+'"></div>');
				else
					$(this.im).wrap('<div class="img-default '+classWidth+'"></div>');
			}
			...
			console.log($(this.im).next('p'));
			$(this.im).next('p').html('im here');
			this.removePane();
		} else if(o.error) {
			this.requestInterval = false;
			this.setProgress(100);
...
var nicCodeButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		this.addForm({
			...
			'code' : {type : 'content', 'value' : this.ne.selectedInstance.getContent(), style : {width: '600px', height : '400px'}}
		});
	},
