	...
	class Editor {
		...
		public function init()
		{
			if ($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				else if ($_POST['action'] == 'DisplayAddLink')
				{
					$this->displayAddLink();
				}
		...
		public function cropImage()
		{
			...
			imagejpeg($newImage, $filepath, 68);
			$tempImage = $newImage;
		...
		{
			list($img, $id) = explode('-', $_POST['mediaId']);
			$Media = $this->getMediaById($id);
			echo '<form>
				<h2>Add Link</h2>
				<input class="form-control" type="text" name="link" value="'.$Media->getLink().'" />
		...
		public function addLink()
		{
			...
			$Media = $this->getMediaById($data['mediaId']);
			$Media->updateLink($link);
			MessageManager::displayMessage('Link Updated');
		}
		...
		public function createPage()
		{
			...
			$header = 1;
			$footer = 1;
			$sql = "INSERT INTO pages (magazineId, sectionId, layoutId, pageNum, title, standfirst, mainCopy, header, footer) VALUES (:magazineId, :sectionId, :layoutId, :pageNum, :title, :standfirst, :content, :header, :footer)";  
			$stmt = $this->mDatabase->prepare($sql);
		...
		public function savePage()
		{
			...
			if (empty($content))
				$content = ' ';
			$id = $this->getPageId();
			$sql = "UPDATE pages SET sectionId = :sectionId, layoutId = :layoutId, title = :title, standfirst = :standfirst, mainCopy = :content WHERE id = :id";  
			$stmt = $this->mDatabase->prepare($sql);
		...
		public function previewPage()
		{
			...
			echo '<script>
   				document.domain = "http://www.pubishing.ninja";
    function setIframeHeight( iframeId )
    {
     ...
     if( ifDoc )
     {
      ...
      ifRef.height = ifDoc.scrollHeight;  
			...
			echo '<iframe width="100%" height="100%" scrolling="no" onload="setIframeHeight( this.id )" id="iframe" frameborder="0" src="http://www.publishing.ninja/publisher/magazine/preview.php?magazineId='.$this->mMagazine->getId().'&pageNum='.$pageNum.'" />';
		}
...
imagejpeg($output, $output_file);*/
		...
		public function formatText(&$content)
		{
			...
			$result = str_replace(array("</h1>","</h3>","</h4>"), "</h2>", $result);
			//Change divs to ps
		...
		public function clean_inside_tags($str,$tags){
			...
			foreach ($allTags[1] as $tag){
				if ($tag != 'img' && $tag != 'div' && $tag != 'a' && $tag != 'td' && $tag!='table')
